...
function cleanup() {
    ...
    # 1. 杀掉通过 PID 记录的进程
    kill -9 $BRIDGE_PID $GAZEBO_PID 2>/dev/null || true
    
    # 2. 无差别强杀残留的同名进程 (防止僵尸进程)
    killall -9 gzserver gzclient qemu-system-arm 2>/dev/null || true
    pkill -9 -f udp_tcp_bridge.py 2>/dev/null || true
...
sed -i 's/<use_tcp>true<\/use_tcp>/<use_tcp>0<\/use_tcp>/g' ./cemu_iris.sdf

echo "🚁 [5/6] 正在向场地投放定制版 Iris 无人机..."
gz model --spawn-file="./cemu_iris.sdf" --model-name=iris -x 0 -y 0 -z 0.1
sleep 2

echo "🌉 [6/6] 启动项目专属网桥 (udp_tcp_bridge.py)..."
python3 udp_tcp_bridge.py &
BRIDGE_PID=$!
sleep 2
